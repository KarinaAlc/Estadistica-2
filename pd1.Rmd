...
```{r, results = 'hide',  fig.show='hide',warning=FALSE,message=FALSE}
filter(usa, relig=="Catolico", intecamp=="1. Mucho") 
...
summarise(usa, MediaAprobGorePost = mean(gorepst, na.rm = T))
```
...
```{r, results = 'hide',  fig.show='hide',warning=FALSE,message=FALSE}
count(usa, relig)
count(usa, educ,sort = FALSE) #numero de personas en base a nivel educativo, con SORT=T estamos diciendo que nos ordene los datos de manera descendente
...
usagoredesc<- mutate(usa, difgore =  gorepst - gorepre) %>% arrange(., desc(gorepst)) %>%
