...
```{r echo = T, results = 'hide',  fig.show='hide'}
modeloRL = lm(accesoInformacion ~ participaciónDigital, Egov)
summary(modeloRL)
```
...
**Descrición** Como su nombre lo dice, debe de existir una linealidad
...
Si usamos la correlación, entonces revisamos cada una de las
...
lineal. ¿Qué pasaría si tuviese que sacar una variable?, ¿Cuál es la más
...
```{r message=FALSE, warning=FALSE, echo = T, results = 'hide',  fig.show='hide'}
library(lmtest)
bptest(modelo1)
```
...
que están **autocorrelacionados** (problema!). En este caso tenemos un p
value de 0.266, mayor a 0.05, el cual nos indica que no nos encontramos
frente a un caso de autocorrelación.
...
2.  ¿Qué tanto explica el modelo? -Revisamos el R cuadrado ajustado que
    va de 0 a 1 (0% a 100%) -En este caso mis variables (en conjunto)
    explican el 91.03% de la variabilidad de mi dependiente, esto es
...
de que es un modelo válido, optaría por mantener mi modelo1. 🚨 **Ojo**,
solo basándome en esta primera evaluación del modelo. Una decisión más
fina sería al realizar mis pruebas de supuestos completa 🚨 🚨 🚨.
