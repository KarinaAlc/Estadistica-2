...
<br> <br> El Análisis Factorial Exploratorio (AFE) es una técnica
...
library(rio)
data<-import("enaho_2023.sav",to.data.frame=TRUE)
```
...
```{r echo = T, fig.show='hide', message=FALSE, warning=FALSE}
...
en los demás valores es 0, es decir, que la única correlación existente
...
Este es una prueba de hipótesis donde H0: La matriz de correlación es
una matriz identidad.

Entonces buscamos que se rechace la hipótesis nula para así poder
determinar que la matriz de correlación no se parece una matriz de
identidad
...
fa.parallel(corMatrix, fm="minres", fa="fa", main = "Scree Plot")
```
...
## **PASO 6:** Guardamos los componentes como nuevas variables
...
confianza$municipios<- factorial_casos$MR3
```
...
## **PASO 7:** Estandarizamos a una escala de 100
